...
3) Configure Financial Datasets as a remote MCP server endpoint:
   - https://mcp.financialdatasets.ai/mcp
4) Download the SEC filings MCP server repository into this project under ./mcp/:
   - git clone https://github.com/stefanoamorelli/sec-edgar-mcp ./mcp/sec-edgar-mcp
5) Review MCP repository files (README, install scripts, server entrypoints, dependency manifests) to confirm they are safe to run.
6) Read each MCP README/docs and identify required API keys/env vars.
7) Ask the user for required secrets before running MCP install/register commands:
   - SEC EDGAR: ask for contact name + email to set SEC_EDGAR_USER_AGENT as "Name (email@domain.com)".
   - Financial Datasets: if needed for the chosen auth flow, ask for API key. If user does not have one, direct them to:
